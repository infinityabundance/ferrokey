...
if grep -q '"event":"key","code":37,"down":true' "$EVENTS" 2>/dev/null; then
...
sleep 1
if grep -q '"event":"key","code":37,"down":false' "$EVENTS" 2>/dev/null; then
...
sleep 1
if grep -q '"event":"key","code":50,"down":false' "$EVENTS" 2>/dev/null; then
