#!/usr/bin/env bash
# One-off debug: focus events with the target's stderr logging enabled.
# Every artifact name is uniquified so stale VMs can never collide.
set -e
TAG=debugfx-$$
STATE=/court/state
...
users:
  - name: court
    ...
    ssh_authorized_keys:
      - "$PUB"
package_update: true
packages: [xserver-xorg-video-dummy, xserver-xorg-core, x11-utils, xdotool, x11-xserver-utils, openbox, jq, procps, python3]
write_files:
...
cat > /tmp/$TAG-focus-debug.sh <<'SEOF'
#!/usr/bin/env bash
Xorg :0 -noreset -nolisten tcp >/tmp/xorg.log 2>&1 &
sleep 3
DISPLAY=:0 openbox >/tmp/openbox.log 2>&1 &
sleep 2
python3 /home/court/payload/courts/recv-events.py /tmp/tgt.sock > /tmp/events.log 2>&1 &
sleep 1
DISPLAY=:0 env TARGET_SOCKET=/tmp/tgt.sock ./payload/bin/ferrokey-test-target-x11 >/tmp/target.log 2>&1 &
sleep 3
echo "=== target.log (events seen) ==="
cat /tmp/target.log
echo "=== events.log ==="
cat /tmp/events.log
W=$(DISPLAY=:0 xdotool search --name ferrokey-test-target | head -1)
DISPLAY=:0 xdotool windowfocus $W
sleep 1
echo "=== after windowfocus: target.log ==="
cat /tmp/target.log
echo "=== events ==="
cat /tmp/events.log
echo "=== xwininfo events ==="
DISPLAY=:0 xwininfo -id $W -events 2>&1 | grep -A20 "Someone wants"
exit 0
SEOF
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -i $KEY -P $SSHPORT /tmp/$TAG-focus-debug.sh court@127.0.0.1:/tmp/focus-debug.sh
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -i $KEY -p $SSHPORT court@127.0.0.1 'timeout 45 bash /tmp/focus-debug.sh' 2>&1 | tail -22 || true
